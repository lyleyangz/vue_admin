  ...
  methods: {
    ...
    _loadConfig() {
      return new Promise((resolve, reject) => {
        ...
        configScript.src = this.mixedConfig.UEDITOR_HOME_URL + "neditor.config.js";
        document.getElementsByTagName("head")[0].appendChild(configScript);
        configScript.onload = function() {
          if (!!window.UE && !!window.UEDITOR_CONFIG && Object.keys(window.UEDITOR_CONFIG).length !== 0) {
            resolve();
          } else {
            console && console.log("加载ueditor.config.js失败,请检查您的配置地址UEDITOR_HOME_URL填写是否正确!");
          }
        };
      });
    },
    ...
    _loadParse() {
      return new Promise((resolve, reject) => {
        let coreScript = document.createElement("script");
        coreScript.type = "text/javascript";
        coreScript.src = this.mixedConfig.UEDITOR_HOME_URL + "neditor.parse.min.js";
        document.getElementsByTagName("head")[0].appendChild(coreScript);
        coreScript.onload = function() {
          resolve();
        };
    ...
    _loadJquery() {
      return new Promise((resolve, reject) => {
        let coreScript = document.createElement("script");
        coreScript.type = "text/javascript";
        coreScript.src = this.mixedConfig.UEDITOR_HOME_URL + "third-party/jquery-1.10.2.min.js";
        document.getElementsByTagName("head")[0].appendChild(coreScript);
        coreScript.onload = function() {
          resolve();
        };
    ...
    _setContent(value) {
      let content = value;
      if (this.isReady) {
        content === this.editor.getContent() || this.editor.setContent(value);
      }
      this.readyValue = content;
    },
    getContentTxt() {
  ...
  watch: {
    value: {
      handler(value) {
        ...
        //将section标签替换成p标签
        if (value) {
          content = value.replace(/<section/g, "<p").replace(/<\/section/g, "</p");
        }
        this.editor ? this._setContent(content) : this._beforeInitEditor(content);
      },
      immediate: true
    }
  }
