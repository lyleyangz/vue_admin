  ...
  UploadImage.prototype = {
    ...
    initUploader: function() {
      var _this = this,
        ...
        acceptExtensions = (editor.getOpt("imageAllowFiles") || [".png", ".jpg", ".jpeg", ".gif", ".bmp"])
          .join("")
          .replace(/\./g, ",")
          .replace(/^[,]/, ""),
      ...
      function ossUpload(files, fileType, uid) {
        this.uid = uid || 1;
        ...
        ajax.request(actionUrl, {
          timeout: 100000,
          data: params,
          method: "get",
          onsuccess: function(res) {
            var response = JSON.parse(res.response);
            var res = JSON.parse(response.bodyMessage);
            var client = new OSS({
              region: res.region,
              secure: true,
            ...
            for (var i = 0; i < files.length; i++) {
              var file = files[i].raw,
              suxfile = file.name.split("."),
              key = res.dir + "/" + generateUUID() + "." + suxfile[1];
              ossArr.push(client.put(key, file));
            }
            Promise.all(ossArr).then(function(res) {
              var queueFiles = uploader.getFiles("queued");
              for (var i = 0; i < queueFiles.length; i++) {
                var cur_file = queueFiles[i];
                cur_file.src = res[i].url;
                cur_file.setStatus("complate");
                uploadImage.imageList.push(cur_file);
                $("body").find("#" + cur_file.id).append('<span class="success"></span>');
              }
            }).catch(error => {
              alert("上传失败");
            });
